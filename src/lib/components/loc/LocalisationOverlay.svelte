...
<OverlaySurface bind:open title="Localisation">
  <TabStrip tier="content" tabs={[{id:"search",label:"Search"},{id:"missing",label:"Missing",count:missingLoaded ? missing.length : undefined}]} activeId={tab} onselect={(id) => (tab = id as typeof tab)} />
...
<style>
  ...
  .pager button { border: 1px solid var(--bg-3); background: var(--bg-2); color: var(--text-1); font-family: inherit; font-size: 0.72rem; padding: 0.15rem 0.5rem; cursor: pointer; }
  .pager button:disabled { opacity: 0.4; cursor: default; }
  .rows { width: 100%; border-collapse: collapse; font-size: 0.78rem; }
  .rows th { text-align: left; color: var(--text-2); font-weight: 600; padding: 0.2rem 0.4rem; border-bottom: 1px solid var(--bg-2); position: sticky; top: 0; background: var(--bg-2); }
  .rows td { padding: 0.12rem 0.4rem; border-bottom: 1px solid var(--bg-1); vertical-align: middle; }
  .rows td.k code { color: var(--ok); background: var(--bg-0); padding: 0 0.3rem; }
  .rows td.v input { width: 100%; background: var(--bg-0); border: 1px solid var(--border); color: var(--text-1); padding: 0.15rem 0.35rem; font-family: inherit; font-size: 0.78rem; }
  td.f { color: var(--text-2); max-width: 16rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .origin { font-size: 0.66rem; border: 1px solid var(--bg-3); padding: 0 0.25rem; color: var(--text-2); }
  .origin.mod { color: var(--ok); border-color: var(--ok); }
  .kind { font-size: 0.68rem; background: var(--accent); color: var(--accent-text); padding: 0 0.3rem; }
  .more, .explain, .dim { color: var(--text-2); font-size: 0.76rem; }
  .explain { margin: 0 0 0.5rem; }
  .mini { border: 1px solid var(--bg-3); background: var(--bg-2); color: var(--text-1); font-family: inherit; font-size: 0.72rem; padding: 0.1rem 0.5rem; cursor: pointer; }
  .mini:hover:not(:disabled) { background: var(--accent); color: var(--text-inverse); }
  .mini:disabled { opacity: 0.5; cursor: default; }
  td.ent { color: var(--text-1); }
</style>
